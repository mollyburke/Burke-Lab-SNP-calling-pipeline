...
paired=TRUE
if [ "$paired" = "TRUE" ]; then
    fqbam="fastqs_to_recalbam-paired.py"
else
    fqbam="fastqs_to_recalbam.py"
fi
...
echo $samples | tr ' ' '\n' | awk -v pys="$fqbam" -v ref="$ref" -v bqsr_vcf="$bqsr_vcf" -v rg_info="$rg_info" '{print pys " --fastq_folder " $1 " --reference_fasta " ref " --bqsr_vcf " bqsr_vcf " --readgroup_info " rg_info }' \
