...
for file in all_files:
    if re.search(r"(.fastq$)|(.fastq.gz$)", file):
        fastq_files.append(file)
        
assert len(fastq_files) > 0, "Error: the folder given, " + args.fastq_folder + ", seems to contain no .fastq or .fastq.gz files?"
...
for fastq in fastq_files:
    samfile = fastq + ".sam"
    cmd = "bwa mem -t 4 -M -R \"" + rg_dic[fastq] + "\" " + args.reference_fasta + " "  + args.fastq_folder + "/" + fastq + " -o " + scratch_dir + "/" + samfile
